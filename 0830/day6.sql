...
/*
   정규표현식 oracle 10g부터 사용가능 REGEXP_ <- 로시작하는 함수
   .(dot) or [] <-- 모든 문자 1글자를 의미함.
   ^시작, $끝 [^] <- 대괄호 안의 ^ not을 의미함.
   {n} :n번반복, {n,}: n이상반복, {n,m} n 이상 m  이하 반복
*/
...
-- mem_mail 데이터 중 영문자 시작3~5 자리 이메일 주소패턴 추출
...
-- 한글만 있는 주소검색
-- *:0회 or 그이상 횟수로, ?:0회 or 1회, +:1회 or 이상으로
SELECT mem_add2
FROM member
--WHERE REGEXP_LIKE(mem_add2, '^[가-힝]+$');
...
--(\d\d)\1$ (숫자숫자) \1 첫번째 그룹 캡처 그룹을 다시참조
